...
//        obliczenie średniej liczby dni, jaka upłynęła od zlecenia wykonania zadania do dnia bieżącego.
//        Uwzględniamy te zadania, które są aktualnie w trakcie realizacji.
        //Given
        Board project = prepareTestData();
        //When
        double averageWorkingOnTask = project.getTaskLists().stream()
                ...
                .map(task -> DAYS.between(task.getCreated(), LocalDate.now()))
                        .mapToInt(Math::toIntExact)
                                .average().orElse(0);
//        możemy użyć dwóch strumieni – jeden aby obliczyć sumę,
//        a drugi, aby obliczyć liczbę elementów. W wersji trudniejszej trzeba wykorzystać kolektor skalarny average().
